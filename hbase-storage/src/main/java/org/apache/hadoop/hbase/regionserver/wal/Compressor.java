...
final public class Compressor {
  ...
  private static void transformFile(Path input, Path output)
    ...
    Reader in = WALUtils.createReaderIgnoreCustomClass(inFS, input, conf);
    Writer out = null;
    ...
    try {
      ...
      out = WALUtils.createWALWriter(outFS, output, conf);
   ...
   *
   * @deprecated Only used in {@link HLogKey} and {@link KeyValueCompression}.
