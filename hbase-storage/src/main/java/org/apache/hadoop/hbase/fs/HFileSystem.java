 ...
 /**
  ...
  static class ReorderWALBlocks implements ReorderBlocks {
    ...
    public void reorderBlocks(Configuration conf, LocatedBlocks lbs, String src)
      ...
      ServerName sn = WALUtils.getServerNameFromWALDirectoryName(conf, src);
      if (sn == null) {
        // It's not an WAL
        return;
      }
   ...
   *                   See hadoop 2.6+ org.apache.hadoop.hdfs.protocol.HdfsConstants
   *                   for possible list
   *                   e.g 'COLD', 'WARM', 'HOT', 'ONE_SSD', 'ALL_SSD', 'LAZY_PERSIST'.
   */
