...
public abstract class FSUtils {
   ...
   * @return Serialized protobuf with <code>version</code> content and
   *         a bit of pb magic for a prefix.
   */
  ...
  public static void checkVersion(FileSystem fs, Path rootdir,
      boolean message, int wait, int retries) throws IOException, DeserializationException {
  ...
  /**
   * Checks for the presence of the WAL log root path (using the provided conf object)
   * in the given path. If
   * it exists, this method removes it and returns the String representation of remaining
   * relative path.
   ...
   * @return {@link Path} to hbase root directory: i.e.
   *         {@value org.apache.hadoop.hbase.HConstants#HBASE_DIR} from
  ...
  /**
   * @param c configuration
   * @return {@link Path} to hbase log root directory:
   *         i.e. {@value org.apache.hadoop.hbase.fs.HFileSystem#HBASE_WAL_DIR} from
   *         configuration as a qualified Path.
   *         Defaults to {@value org.apache.hadoop.hbase.HConstants#HBASE_DIR}
   * @throws IOException e
   */
  ...
  public static List<Path> getTableDirs(final FileSystem fs, final Path rootdir)
    ...
    for(FileStatus status : fs.globStatus(new Path(rootdir,
   ...
   *         Ignore non table hbase folders such as .logs, .oldlogs, .corrupt folders.
   */
  ...
  public static Path getRegionDirFromTableDir(Path tableDir, HRegionInfo region) {
    HRegionInfo regionInfo = region == null ? null :
      RegionReplicaUtil.getRegionInfoForDefaultReplica(region);
    return new Path(tableDir, regionInfo.getEncodedName());
  }
  ...
  private static void getRegionLocalityMappingFromFS(
    ...
    if (null == desiredTable) {
      queryPath = new Path(
        new Path(rootPath, HConstants.BASE_NAMESPACE_DIR).toString() + "/*/*/*/");
    } else {
      queryPath = new Path(
        FSUtils.getTableDir(rootPath, TableName.valueOf(desiredTable)).toString() + "/*/");
    }
