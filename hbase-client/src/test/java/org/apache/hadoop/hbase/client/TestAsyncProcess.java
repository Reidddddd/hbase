...
public class TestAsyncProcess {
  ...
  // Replica stuff
  private static final HRegionInfo hri1r1 =
      RegionReplicaUtil.getRegionInfoForReplica(hri1, 1);
  private static final HRegionInfo hri1r2 =
      RegionReplicaUtil.getRegionInfoForReplica(hri1, 2);
  private static final RegionLocations hrls1 =
  ...
  private static final HRegionInfo hri2r1 =
  ...
  private static final Exception failure = new Exception("failure");
  ...
  private static long calculateRequestCount(long putSizePerServer,
  ...
  private static ClusterConnection createHConnectionWithReplicas()
      throws IOException {
    ClusterConnection hc = createHConnectionCommon();
  ...
  private static BufferedMutatorParams createBufferedMutatorParams(
      MyAsyncProcess ap, TableName name) {
    return new BufferedMutatorParams(name).pool(ap.pool);
  }
  ...
  public void testIllegalArgument() throws IOException {
    ...
    final long maxHeapSizePerRequest = conn.getConfiguration()
        .getLong(AsyncProcess.HBASE_CLIENT_MAX_PERREQUEST_HEAPSIZE,
            AsyncProcess.DEFAULT_HBASE_CLIENT_MAX_PERREQUEST_HEAPSIZE);
    conn.getConfiguration()
        .setLong(AsyncProcess.HBASE_CLIENT_MAX_PERREQUEST_HEAPSIZE, -1);
    try {
  ...
  private void doSubmitRequest(long maxHeapSizePerRequest, long putsHeapSize)
      throws Exception {
    ClusterConnection conn = createHConnection();
    final long defaultHeapSizePerRequest = conn.getConfiguration()
        .getLong(AsyncProcess.HBASE_CLIENT_MAX_PERREQUEST_HEAPSIZE,
            AsyncProcess.DEFAULT_HBASE_CLIENT_MAX_PERREQUEST_HEAPSIZE);
    conn.getConfiguration()
        .setLong(AsyncProcess.HBASE_CLIENT_MAX_PERREQUEST_HEAPSIZE,
    ...
    int minCountSnRequest =
        (int) calculateRequestCount(putSizeSN, maxHeapSizePerRequest);
    int minCountSn2Request =
        (int) calculateRequestCount(putSizeSN2, maxHeapSizePerRequest);
    ...
    try (BufferedMutatorImpl mutator = new BufferedMutatorImpl(conn, null, null, bufferParam)) {
      ...
      for (AsyncRequestFuture req : reqs) {
        ...
        MyAsyncProcess.MyAsyncRequestFutureImpl ars =
            (MyAsyncProcess.MyAsyncRequestFutureImpl) req;
  ...
  public void testSubmitWithCB() throws Exception {
    ...
    final AsyncRequestFuture ars =
        ap.submit(DUMMY_TABLE, puts, false, cb, false);
    Assert.assertTrue(puts.isEmpty());
    ars.waitUntilDone();
  ...
  public void testSubmitBusyRegionServer() throws Exception {
    ...
    ap.taskCounterPerServer.put(sn2,
        new AtomicInteger(ap.maxConcurrentTasksPerServer));

    List<Put> puts = new ArrayList<Put>();
    puts.add(createPut(1, true));
    puts.add(createPut(3, true)); // <== this one won't be taken, the rs is busy
    puts.add(createPut(1,
        true)); // <== this one will make it, the region is already in
    ...
    ap.taskCounterPerServer.put(sn2,
        new AtomicInteger(ap.maxConcurrentTasksPerServer - 1));
    ap.submit(DUMMY_TABLE, puts, false, null, false);
    Assert.assertTrue(puts.isEmpty());
  }
  ...
  public void testSubmitTrue() throws IOException {
    final AsyncProcess ap =
        new MyAsyncProcess(createHConnection(), conf, false);
  ...
  @Test
  public void testTaskCountWithoutClientBackoffPolicy()
  ...
  @Test
  public void testTaskCountWithClientBackoffPolicy()
      throws IOException, InterruptedException {
    ...
    Mockito.when(hc.getStatisticsTracker())
        .thenReturn(ServerStatisticTracker.create(copyConf));
    Mockito.when(hc.getBackoffPolicy()).thenReturn(bp);
    MyAsyncProcess ap = new MyAsyncProcess(hc, copyConf, false);
    testTaskCount(ap);
  }

  private void testTaskCount(AsyncProcess ap)
    ...
    Thread t2 = new Thread() {
      ...
      public void run() {
        ...
        while (ap.tasksInProgress.get() > 0) {
          ap.decTaskCounters(Collections.singletonList(Bytes.toBytes("dummy")),
              sn);
        }
      }
    };
  ...
  public void testHTableFailedPutAndNewPut() throws Exception {
    ...
    Assert.assertEquals("the put should not been inserted.", 0,
        mutator.getWriteBuffer().size());
  }
  ...
  private void checkPeriodicFlushParameters(ClusterConnection conn,
      MyAsyncProcess ap, long setTO, long expectTO, long setTT, long expectTT) {
    BufferedMutatorParams bufferParam =
        createBufferedMutatorParams(ap, DUMMY_TABLE);
    ...
    Assert.assertEquals(setTO,
        bufferParam.getWriteBufferPeriodicFlushTimeoutMs());
    Assert.assertEquals(setTT,
        bufferParam.getWriteBufferPeriodicFlushTimerTickMs());

    // The BufferedMutatorImpl corrects illegal values (indirect via BufferedMutatorParams)
    BufferedMutatorImpl ht1 =
        new BufferedMutatorImpl(conn, null, null, bufferParam);
    ...
    BufferedMutatorImpl ht2 = new BufferedMutatorImpl(conn, null, null,
        createBufferedMutatorParams(ap, DUMMY_TABLE));
  ...
  public void testWriteBufferPeriodicFlushTimeoutMs() throws Exception {
    ...
    bufferParam.writeBufferSize(
        10000);  // Write buffer set to much larger than the single record

    BufferedMutatorImpl ht =
        new BufferedMutatorImpl(conn, null, null, bufferParam);
    ht.ap = ap;
  ...
  public void testTaskCheckerHost() throws IOException {
    ...
    // unlimiited
    RequestSizeChecker sizeChecker =
        new RequestSizeChecker(maxHeapSizePerRequest);
    RowCheckerHost checkerHost =
        new RowCheckerHost(Arrays.asList(countChecker, sizeChecker));

    ReturnCode loc1Code =
        checkerHost.canTakeOperation(loc1, maxHeapSizePerRequest);
    assertEquals(RowChecker.ReturnCode.INCLUDE, loc1Code);

    ReturnCode loc1Code_2 =
        checkerHost.canTakeOperation(loc1, maxHeapSizePerRequest);
    // rejected for size
    assertNotEquals(RowChecker.ReturnCode.INCLUDE, loc1Code_2);

    ReturnCode loc2Code =
        checkerHost.canTakeOperation(loc2, maxHeapSizePerRequest);
    // rejected for size
    assertNotEquals(RowChecker.ReturnCode.INCLUDE, loc2Code);

    // fill the task slots for loc3.
    taskCounterPerRegion.put(loc3.getRegionInfo().getRegionName(),
  ...
  public void testRequestSizeCheckerr() throws IOException {
    ...
    ReturnCode acceptCode =
        checker.canTakeOperation(loc1, maxHeapSizePerRequest);
  ...
  public void testTaskCountChecker() throws InterruptedIOException {
    ...
    TaskCountChecker checker = new TaskCountChecker(maxTotalConcurrentTasks,
        maxConcurrentTasksPerServer, maxConcurrentTasksPerRegion,
        tasksInProgress, taskCounterPerServer, taskCounterPerRegion);
    ...
    taskCounterPerRegion.put(loc1.getRegionInfo().getRegionName(),
        new AtomicInteger(100));
  ...
  public void testErrorsServers() throws IOException {
    ...
    BufferedMutatorImpl mutator = new BufferedMutatorImpl(conn, null, null,
        new BufferedMutatorParams(DUMMY_TABLE));
  ...
  public void testGlobalErrors() throws IOException {
    ...
    AsyncProcessWithFailure ap =
        new AsyncProcessWithFailure(conn, conf, new IOException("test"));
    mutator.ap = ap;
  ...
  public void testReplicaReplicaSuccess() throws Exception {
    ...
    List<Get> rows =
        makeTimelineGets(DUMMY_BYTES_1, DUMMY_BYTES_2, DUMMY_BYTES_3);
    AsyncRequestFuture ars =
        ap.submitAll(DUMMY_TABLE, rows, null, new Object[3]);
  ...
  public void testReplicaParallelCallsSucceed() throws Exception {
    ...
    AsyncRequestFuture ars =
        ap.submitAll(DUMMY_TABLE, rows, null, new Object[2]);
  ...
  public void testReplicaAllCallsFailForOneRegion() throws Exception {
    ...
    for (int i = 0; i < ars.getErrors().getNumExceptions(); ++i) {
      Assert.assertArrayEquals(DUMMY_BYTES_1,
          ars.getErrors().getRow(i).getRow());
    }
  }

  private MyAsyncProcessWithReplicas createReplicaAp(int replicaAfterMs,
      int primaryMs, int replicaMs) throws Exception {
    return createReplicaAp(replicaAfterMs, primaryMs, replicaMs, -1);
  }

  private MyAsyncProcessWithReplicas createReplicaAp(int replicaAfterMs,
      int primaryMs, int replicaMs, int retries) throws Exception {
  ...
  private void verifyReplicaResult(AsyncRequestFuture ars, RR... expecteds)
      throws Exception {
    ...
    for (int i = 0; i < expecteds.length; ++i) {
      ...
      Assert.assertEquals(actual.toString(), expected == RR.FAILED,
          actual instanceof Throwable);
  ...
  public void testRetryPauseWithCallQueueTooBigException() throws Exception {
    ...
    LOG.debug("Expected to sleep " + expectedSleep + "ms, actually slept "
        + actualSleep + "ms");
    Assert.assertTrue(
        "Expected to sleep " + expectedSleep + " but actually " + actualSleep
            + "ms", actualSleep >= expectedSleep);

    // check and confirm normal IOE will use the normal pause
    final long normalPause = myConf.getLong(HConstants.HBASE_CLIENT_PAUSE,
    ...
    LOG.debug("Expected to sleep " + expectedSleep + "ms, actually slept "
        + actualSleep + "ms");
    Assert.assertTrue("Slept for too long: " + actualSleep + "ms",
  ...
  public void testQueueRowAccess() throws Exception {
    ...
    Put p0 = new Put(DUMMY_BYTES_1).addColumn(DUMMY_BYTES_1, DUMMY_BYTES_1,
        DUMMY_BYTES_1);
    Put p1 = new Put(DUMMY_BYTES_2).addColumn(DUMMY_BYTES_2, DUMMY_BYTES_2,
        DUMMY_BYTES_2);
    mutator.mutate(p0);
  ...
  static class MyAsyncProcess extends AsyncProcess {
    ...
    public List<AsyncRequestFuture> allReqs =
        new ArrayList<AsyncRequestFuture>();
    ...
    public MyAsyncProcess(ClusterConnection hc, Configuration conf,
      ...
      super(hc, conf, new ThreadPoolExecutor(1, 20, 60, TimeUnit.SECONDS,
          ...
          new RpcControllerFactory(conf), rpcTimeout);
    }
    ...
    public MyAsyncProcess(ClusterConnection hc, Configuration conf,
      ...
      super(hc, conf, new ThreadPoolExecutor(1, 20, 60, TimeUnit.SECONDS,
              new SynchronousQueue<Runnable>(),
              new CountingThreadFactory(new AtomicInteger())),
          new RpcRetryingCallerFactory(conf), useGlobalErrors,
          new RpcControllerFactory(conf), rpcTimeout);
    }

    public MyAsyncProcess(ClusterConnection hc, Configuration conf,
        boolean useGlobalErrors, @SuppressWarnings("unused") boolean dummy) {
      super(hc, conf, new ThreadPoolExecutor(1, 20, 60, TimeUnit.SECONDS,
              new SynchronousQueue<Runnable>(),
              new CountingThreadFactory(new AtomicInteger())) {
            @Override
    ...
    @Override
    <Res> AsyncRequestFutureImpl<Res> createAsyncRequestFuture(
        TableName tableName, List<Action<Row>> actions, long nonceGroup,
        ExecutorService pool, Batch.Callback<Res> callback, Object[] results,
        boolean needResults, PayloadCarryingServerCallable callable,
        int operationTimeout, int rpcTimeout) {
      // Test HTable has tableName of null, so pass DUMMY_TABLE
      MyAsyncRequestFutureImpl<Res> r =
          new MyAsyncRequestFutureImpl(DUMMY_TABLE, actions, nonceGroup,
    ...
    @Override
    public <Res> AsyncRequestFuture submit(TableName tableName,
        RowAccess<? extends Row> rows, boolean atLeastOne,
        Callback<Res> callback, boolean needResults)
    ...
    @Override
    public <Res> AsyncRequestFuture submit(TableName tableName,
        List<? extends Row> rows, boolean atLeastOne, Callback<Res> callback,
    ...
    public <CResult> AsyncRequestFuture submitAll(ExecutorService pool,
        TableName tableName, List<? extends Row> rows,
        Batch.Callback<CResult> callback, Object[] results,
        PayloadCarryingServerCallable callable, int operationTimeout,
        int rpcTimeout) {
      previousTimeout = rpcTimeout;
      return super.submitAll(pool, tableName, rows, callback, results, callable,
          operationTimeout, rpcTimeout);
    }

    @Override
    protected void updateStats(ServerName server,
    ...
    protected RpcRetryingCaller<MultiResponse> createCaller(
      ...
      final MultiResponse mr =
          createMultiResponse(callable1.getMulti(), nbMultiResponse, nbActions,
      ...
      return new RpcRetryingCaller<MultiResponse>(100, 500, 10, 9) {
        ...
        public MultiResponse callWithoutRetries(
            RetryingCallable<MultiResponse> callable, int callTimeout)
    ...
    class MyAsyncRequestFutureImpl<Res> extends AsyncRequestFutureImpl<Res> {
      ...
      MyAsyncRequestFutureImpl(TableName tableName, List<Action<Row>> actions,
          long nonceGroup, ExecutorService pool, boolean needResults,
          Object[] results, Batch.Callback<Res> callback,
          PayloadCarryingServerCallable callable, int operationTimeout,
          int rpcTimeout) {
        super(tableName, actions, nonceGroup, pool, needResults, results,
            callback, callable, operationTimeout, rpcTimeout);
      }
  ...
  static class MyAsyncProcessWithReplicas extends MyAsyncProcess {
    ...
    private final Set<byte[]> failures =
        new TreeSet<byte[]>(new Bytes.ByteArrayComparator());
    private long primarySleepMs = 0, replicaSleepMs = 0;
    private final Map<ServerName, Long> customPrimarySleepMs =
        new HashMap<ServerName, Long>();

    public MyAsyncProcessWithReplicas(ClusterConnection hc,
        Configuration conf) {
      super(hc, conf);
    }
    ...
    protected RpcRetryingCaller<MultiResponse> createCaller(
      ...
      final MultiResponse mr =
          createMultiResponse(callable.getMulti(), nbMultiResponse, nbActions,
              new ResponseGenerator() {
                @Override
                public void addResponse(MultiResponse mr, byte[] regionName,
                    Action<Row> a) {
                  if (failures.contains(regionName)) {
                    ...
                    boolean isStale =
                        !RegionReplicaUtil.isDefaultReplica(a.getReplicaId());
      ...
      final boolean isDefault = RegionReplicaUtil.isDefaultReplica(
          callable.getMulti().actions.values().iterator().next().iterator()
              .next().getReplicaId());
      final ServerName server =
          callable.getServerName();
      String debugMsg =
          "Call to " + server + ", primary=" + isDefault + " with "
      ...
      return new RpcRetryingCaller<MultiResponse>(100, 500, 10, 9) {
        ...
        public MultiResponse callWithoutRetries(
          ...
          if (isDefault) {
            ...
            sleep = (customSleep == null ? primarySleepMs :
                customSleep.longValue());
          } else {
  ...
  static class MyConnectionImpl
    ...
    private static Configuration setupConf(Configuration conf) {
      conf.setClass(RegistryFactory.REGISTRY_IMPL_CONF_KEY, TestRegistry.class,
          Registry.class);
      return conf;
    }
  ...
  static class MyThreadPoolExecutor extends ThreadPoolExecutor {
    public MyThreadPoolExecutor(int coreThreads, int maxThreads,
  ...
  static class AsyncProcessForThrowableCheck extends AsyncProcess {
    ...
    public AsyncProcessForThrowableCheck(ClusterConnection hc,
        Configuration conf, ExecutorService pool) {
      super(hc, conf, pool, new RpcRetryingCallerFactory(conf), false,
          new RpcControllerFactory(conf), rpcTimeout);
    }
  }
}
