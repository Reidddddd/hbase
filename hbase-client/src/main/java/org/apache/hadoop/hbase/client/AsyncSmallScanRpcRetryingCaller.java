...
class AsyncSmallScanRpcRetryingCaller {
  ...
  public AsyncSmallScanRpcRetryingCaller(AsyncConnectionImpl conn, TableName tableName, Scan scan,
                                         int limit, long pauseNs, int maxAttempts, long scanTimeoutNs,
                                         long rpcTimeoutNs, int startLogErrosCnt) {
    this.conn = conn;
  ...
  private static final class SmallScanResponse {
    ...
    private final boolean hasMoreResultsInRegion;
    ...
    stub.scan(controller, req, resp -> {
      if (controller.failed()) {
        ...
        try {
          ...
          future.complete(
                  new SmallScanResponse(results, loc.getRegionInfo(),
                          resp.getMoreResultsInRegion()));
        } catch (IOException e) {
          future.completeExceptionally(e);
        }
      }
