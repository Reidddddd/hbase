...
class RawAsyncTableImpl implements RawAsyncTable {
  ...
  public RawAsyncTableImpl(AsyncConnectionImpl conn, AsyncTableBuilderBase<?> builder) {
    this.conn = conn;
  ...
  public CompletableFuture<Void> mutateRow(RowMutations mutation) {
    return this.<Void> newCaller(mutation, writeRpcTimeoutNs).action((controller, loc, stub) ->
                    RawAsyncTableImpl.<Void> mutateRow(controller, loc, stub, mutation, (rn, rm)
                            -> {
