...
public interface RawAsyncTable extends AsyncTableBase {
   ...
   * {@link #coprocessorService(Function, CoprocessorCallable, byte[], boolean, byte[], boolean,
   * CoprocessorCallback)} for more details.
   * @see #coprocessorService(Function, CoprocessorCallable, byte[], boolean, byte[], boolean,
   *      CoprocessorCallback)
   */
  default <S, R> void coprocessorService(
          Function<RpcChannel, S> stubMaker, CoprocessorCallable<S, R> callable, byte[] startKey,
          byte[] endKey, CoprocessorCallback<R> callback) {
