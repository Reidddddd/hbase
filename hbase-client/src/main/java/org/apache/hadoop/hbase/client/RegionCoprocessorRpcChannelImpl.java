...
class RegionCoprocessorRpcChannelImpl implements RpcChannel {
  ...
  private CompletableFuture<Message> rpcCall(
          MethodDescriptor method, Message request, Message responsePrototype,
    ...
    CoprocessorServiceRequest csr = CoprocessorServiceRequest.newBuilder()
            .setCall(call).setRegion(
                    RequestConverter.buildRegionSpecifier(REGION_NAME,
                            loc.getRegionInfo().getRegionName()))
            .build();
    stub.execService(controller, csr,
            new com.google.protobuf.RpcCallback<CoprocessorServiceResponse>() {
          ...
          if (controller.failed()) {
            ...
            try {
              ...
              if (resp.getValue().hasValue()) {
                ...
                ProtobufUtil.mergeFrom(builder, resp.getValue().getValue());
