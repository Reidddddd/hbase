...
public class TestRSGroupAutoMoving extends TestRSGroupsBase {
  ...
  public void testGroupAutoMoving() throws IOException, InterruptedException {
    ...
    Configuration oldConf = cluster.getConf();
    Configuration conf = HBaseConfiguration.create(TEST_UTIL.getConfiguration());

    conf.set("hbase.regionserver.group", rsgroupName);
    conf.setBoolean("hbase.k8s.enabled", true);
    cluster.setConf(conf);

    JVMClusterUtil.RegionServerThread rsThread1 = cluster.startRegionServer();
    // Sleep one second to wait the rsgroup table updated.
    Thread.sleep(1000);
