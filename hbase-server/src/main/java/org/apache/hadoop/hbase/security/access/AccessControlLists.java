    ...
    if (t == null) {
      try (Connection connection = ConnectionFactory.createConnection(conf)) {
        try (Table table = connection.getTable(ACL_TABLE_NAME)) {
          table.put(Collections.singletonList(p));
        }
      }
    } else {
      try {
        /**
         * TODO: Use Table.put(Put) instead. This Table.put() happens within the RS.
         * We are already in AccessController. Means already there was an RPC happened
         * to server (Actual grant call from client side). At RpcServer we have a ThreadLocal
         * where we keep the CallContext and inside that the current RPC called user info is set.
         * The table on which put was called is created via the RegionCP env
         * and that uses a special Connection. The normal RPC channel will be by passed here means
         * there would have no further contact on to the RpcServer.
         * So the ThreadLocal is never getting reset.
         * We ran the new put as a super user
         * (User.runAsLoginUser where the login user is the user who started RS process)
         * but still as per the RPC context it is the old user.
         * When AsyncProcess was used, the execute happen via another thread from
         * pool and so old ThreadLocal variable is not accessible and so it looks as
         * if no Rpc context and we were relying on the super user who starts the RS process.
         */
