...
/**
 * RPC Executor that combines the characters of {@link RWQueueRpcExecutor} and
...
public class FastPathRWQueueRpcExecutor extends RWQueueRpcExecutor {
  private static final Log LOG = LogFactory.getLog(FastPathRWQueueRpcExecutor.class);
  ...
  protected RpcHandler getHandler(final String name, final double handlerFailureThreshhold,
    ...
    return new FastPathRpcHandler(name, handlerFailureThreshhold, q, activeHandlerCount,
        failedHandlerCount, handlerCount, abortable, handlerStack);
  }

  @Override
  public boolean dispatch(CallRunner callTask) throws InterruptedException {
    RpcServer.Call call = callTask.getCall();
    boolean isWriteRequest = isWriteRequest(call.getHeader(), call.param);
    boolean shouldDispatchToScanQueue = shouldDispatchToScanQueue(callTask);
    FastPathRpcHandler handler = isWriteRequest ?
        writeHandlerStack.poll() : shouldDispatchToScanQueue ?
        scanHandlerStack.poll() : readHandlerStack.poll();
