...
public class CredentialCache {
      ...
      public void run() {
        ...
        while (!decryptor.isInitialized()) {
          ...
          try {
            ...
            ClusterConnection conn = server.getClusterConnection();
            while (!conn.isTableAvailable(SecretTableAccessor.getSecretTableName(), null)) {
              Thread.sleep(1000);
            }
            Thread.sleep(1000);
  ...
  private Table getAuthTable() throws IOException {
    if (authTable.get() == null) {
      ClusterConnection connection = server.getClusterConnection();
      if (connection == null) {
  ...
  private void refreshCredential() throws IOException {
    ...
    ClusterConnection conn = this.server.getClusterConnection();
