...
public class AccessController extends BaseMasterAndRegionObserver
  ...
  public Cell postMutationBeforeWAL(ObserverContext<RegionCoprocessorEnvironment> ctx,
    ...
    // As Increment and Append operations have clone the tags of oldCell to newCell, there is no need
    // to copy them again. Just extract non-acl tags of newCell if we need to add a new acl tag for
    // the cell. Actually, oldCell is useless here.
    List<Tag> tags = Lists.newArrayList();
    ListMultimap<String,Permission> perms = ArrayListMultimap.create();
    if (newCell != null) {
      // Save an object allocation where we can
      if (newCell.getTagsLength() > 0) {
        Iterator<Tag> tagIterator = CellUtil.tagsIterator(newCell.getTagsArray(),
    ...
    // mutation.getACL() must not be null here.
