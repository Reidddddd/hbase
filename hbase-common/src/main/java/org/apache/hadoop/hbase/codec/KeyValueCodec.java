...
public class KeyValueCodec extends AbstractCodec {
  ...
  public static class ByteBufferKeyValueDecoder implements Codec.Decoder  {
    final HandlerLAB bufferPool;
    HandlerLAB.ByteArrayAndOffset arrayAndOffset;
    Cell current;
    ByteBuffer buf;

    public ByteBufferKeyValueDecoder(ByteBuffer buf) {
      this.buf = buf;
      this.bufferPool = HandlerLAB.getInstance();
    }
    ...
    public boolean advance() throws IOException {
      ...
      arrayAndOffset = bufferPool.claimBytesFromHandlerLAB(len);
      ByteBufferUtils.copyFromBufferToArray(arrayAndOffset.array(), buf,
          currentPos, arrayAndOffset.offset(), len);
      current = new KeyValue(arrayAndOffset.array(), arrayAndOffset.offset(), len);
