...
public class ByteBuffInputStream extends InputStream  {
  ...
  public ByteBuffInputStream(final ByteBuffer buffer, final IPCReservoir reservoir) {
    this.buffer = buffer;
  ...
  public int read(byte[] b, int off, int len) {
    ...
    ByteBufferUtils.copyFromBufferToArray(b, buffer, buffer.position(), off, len);
    buffer.position(buffer.position() + len); // we should advance the buffer position
    return len;
  }
