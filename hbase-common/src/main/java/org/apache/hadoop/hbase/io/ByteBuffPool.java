      ...
      timer.scheduleAtFixedRate(new TimerTask() {
        ...
        public void run() {
          ...
          for (BufferSizeManager bsm : bufferManagers.values()) {
            if (currentT - bsm.lastTouch > 7200000) {
              // hard code: 2 hours.
              // Do not want to introduce parameters which branch-2+s don't have
              // the time should be long enough for a normal job
              // not too short to trigger too many re-creation.
              bsm.lock.lock();
              try {
                bsm.pool.poll(); // Remove it since it hasn't been claimed for more than 2 hours.
              } finally {
                bsm.lock.unlock();
                bsm.touch();
                cleaned = true;
              ...
              // We only remove one from one of the BufferSizeManager at one time, to be gently.
              break;
            }
          }
          if (!cleaned) {
    ...
    if (runCleanTask) {
      this.timer.schedule(new TimerTask() {
        ...
        public void run() {
          ...
          long currentT = EnvironmentEdgeManager.currentTime();
          for (BufferSizeManager bsm : bufferManagers.values()) {
            if (currentT - bsm.lastTouch > 5000) {
              // for test: 5 seconds.
              bsm.lock.lock();
              try {
                bsm.pool.poll(); // Remove it since it hasn't been claimed for more than 2 hours.
      ...
      }, 10000, 6000); // delay 10 seconds, run every 6 seconds.
    }
  }
